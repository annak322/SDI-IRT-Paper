...
*****************************************************************************
	...
	use "${root}/SDI-Health/harmonizedData/SDI_AllCountries_Vignettes.dta", clear
	merge 1:1 survey_id using "${root}/irt_output_afterto$recodeAfter.dta"
	assert _merge==3
	drop _merge
	tempfile theData
...
*****************************************************************************
	...
	foreach country in $theCountries {
		use "${root}/SDI-Health/harmonizedData/SDI_`country'_Facility.dta", clear
...
*****************************************************************************
	...
	foreach country in $theCountries {
		use "${root}/SDI-Health/harmonizedData/SDI_`country'_Absenteeism.dta", clear
...
*****************************************************************************
	...
	//Code correct treatments
		egen diarrhea_correctt = anymatch(diarrhea_treat_ivfluids diarrhea_treat_ngtube diarrhea_treat_tubedose), val(1)
		replace diarrhea_correctt = 1 if (diarrhea_refer_facility==1 | diarrhea_refer_clinician==1) & facility_level==1 //allow referral in health post
		...
		replace pneumonia_correctt = 1 if (pneumonia_refer_facility==1 | pneumonia_refer_clinician==1) & facility_level==1 //allow referral in health post

		egen diabetes_correctt = anymatch(diabetes_treat_hypoglycmcs diabetes_refer_specialist diabetes_refer_clinician tb_refer_facility tb_refer_clinician), val(1)
		
		egen tb_correctt = anymatch(tb_treat_ctdurdose tb_treat_ctdose tb_treat_ctdrugs tb_treat_ctdur tb_refer_tbclinic tb_refer_facility tb_refer_clinician), val(1)
		...
		replace malaria_correctt = 1 if (malaria_refer_facility==1 | malaria_refer_clinician==1) & facility_level==1 //allow referral in health post
		...
		replace pph_correctt = 1 if (pph_refer_facility==1 | pph_refer_clinician==1) & facility_level==1 //allow for referral in health post
		...
		replace asphyxia_correctt = 1 if (asphyxia_refer_facility==1 | asphyxia_refer_clinician==1) & facility_level==1 //allow referral in health post
...
*****************************************************************************
	...
	compress
	saveold "${root}/SDI_Vignette_IRT_Analysis_AfterTo$recodeAfter.dta", replace v(12)
